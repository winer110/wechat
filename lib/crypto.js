...
module.exports = {
  decrypt: (code, callback) => {
    fs.readFile(path.join(__dirname, '/key/first_private_key'), 'utf8', (err, privateKey) => {
      if (err) {
  ...
  encrypt: (code, callback) => {
    fs.readFile(path.join(__dirname, '/key/second_public_key'), 'utf8', (err, publicKey) => {
      if (err) {
        callback(err)
      } else {
